...
import { getReportTemplates, type ReportTemplate } from "@/shared/api/reportTemplates";
    ...
    try {
      ...
      if (format === "json") {
        const response = await apiClient.get(`/agent-tasks/${task.id}/report`, {
          params: { format: "json", template_id: selectedTemplateId === "system-default" ? undefined : selectedTemplateId },
        });
        content = JSON.stringify(response.data, null, 2);
      } else if (format === "html") {
        const mdResponse = await apiClient.get(`/agent-tasks/${task.id}/report`, {
          params: { format: "markdown", template_id: selectedTemplateId === "system-default" ? undefined : selectedTemplateId },
          responseType: "text",
        });
...
<body>
    ...
    <div class="container">
      <div class="header-content">
        <div class="brand">
          ...
          <span class="brand-text">AuditAI</span>
        </div>
  ...
  useEffect(() => {
    if (open && task) {
      ...
      getReportTemplates().then((response) => {
        setReportTemplates(response.items);
        const defaultTemplate = response.items.find((item) => item.is_default) || response.items[0];
  ...
  const handleDownload = async () => {
    ...
    setDownloading(true);
    try {
      await downloadAgentReport(task.id, activeFormat, selectedTemplateId === "system-default" ? undefined : selectedTemplateId);
      setDownloadSuccess(true);
      toast.success(`报告已导出为 ${FORMAT_CONFIG[activeFormat].label} 格式`);
      setTimeout(() => {
        onOpenChange(false);
      }, 1000);
    } catch (err) {
            ...
            <div className="w-72 flex-shrink-0 border-r border-border bg-card/50 p-4 space-y-4 overflow-y-auto">
              ...
              <div className="rounded-2xl border border-border bg-card/70 p-4 space-y-3">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-sm font-medium text-foreground">报告模板</p>
                    <p className="text-xs text-muted-foreground">任务完成后已自动生成最终漏洞报告，这里可以切换导出模板。</p>
                  </div>
                </div>
                <Select value={selectedTemplateId} onValueChange={(value) => { setSelectedTemplateId(value); previewCache.current.clear(); fetchPreview(activeFormat, true); }}>
                  <SelectTrigger className="bg-background/80"><SelectValue placeholder="选择报告模板" /></SelectTrigger>
                  <SelectContent>
