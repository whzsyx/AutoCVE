...
from app.services.agent.tools import (
    FileReadTool, FileSearchTool, ListFilesTool, ReadManyFilesTool,
    PatternMatchTool,
)
...
class TestFileTools:
    ...
    @pytest.mark.asyncio
    async def test_file_search_tool_accepts_query_alias(self, temp_project_dir):
        tool = FileSearchTool(temp_project_dir)

        result = await tool.execute(query="cursor.execute")
    ...
    async def test_read_many_files_tool_reads_multiple_files(self, temp_project_dir):
        ...
        result = await tool.execute(
            file_paths=["src/sql_vuln.py", "src/cmd_vuln.py"],
            max_files=4,
        )

        assert result.success is True
        assert "src/sql_vuln.py" in result.data
        assert "src/cmd_vuln.py" in result.data
        assert result.metadata["files_read"] == ["src/sql_vuln.py", "src/cmd_vuln.py"]

    @pytest.mark.asyncio
    async def test_read_many_files_tool_blocks_path_traversal(self, temp_project_dir):
        tool = ReadManyFilesTool(temp_project_dir)

        result = await tool.execute(
            file_paths=["src/sql_vuln.py", "../../../etc/passwd"],
            max_files=4,
        )

        assert result.success is False
        assert "failed" in result.error.lower() or "security" in result.error.lower()
    ...
    @pytest.mark.asyncio
    async def test_file_read_tool_allows_skill_library_additional_root(self, tmp_path):
        project_root = tmp_path / "audit-target"
        project_root.mkdir()
        skill_root = tmp_path / "skill_library" / "code-audit-finding"
        skill_root.mkdir(parents=True)
        skill_file = skill_root / "SKILL.md"
        skill_file.write_text("Skill body", encoding="utf-8")

        tool = FileReadTool(str(project_root), additional_roots=[str(tmp_path / "skill_library")])

        result = await tool.execute(file_path=str(skill_file))

        assert result.success is True
        assert "Skill body" in result.data

    @pytest.mark.asyncio
    async def test_list_files_tool_allows_skill_references_additional_root(self, tmp_path):
        project_root = tmp_path / "audit-target"
        project_root.mkdir()
        references_root = tmp_path / "skill_library" / "code-audit-finding" / "references"
        references_root.mkdir(parents=True)
        (references_root / "guide.md").write_text("reference", encoding="utf-8")

        tool = ListFilesTool(str(project_root), additional_roots=[str(tmp_path / "skill_library")])

        result = await tool.execute(directory=str(references_root), recursive=True)
