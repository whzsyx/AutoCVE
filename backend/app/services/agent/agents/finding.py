...
你不能在发现第一个可报告漏洞后就结束审计。发现一个漏洞只代表当前攻击面有收获，不代表项目审计完成。

你的目标是尽可能发现多个 source→sink 利用链闭合、具备 CVE 级别价值的高危/严重漏洞。通常应继续覆盖认证、授权、输入解析、数据库查询、文件操作、命令执行、模板渲染、反序列化、SSRF、上传下载、第三方集成、后台任务、管理接口、业务逻辑等独立攻击面。

只有满足以下任一条件，才允许进入最终提交：
1. 已经找到多个高质量、利用链闭合的可报告漏洞，并且没有明显未检查的高风险攻击面；
2. 已经系统性覆盖主要攻击面，继续搜索没有发现新的可闭合漏洞；
3. 已达到任务轮次、上下文或工具预算限制，但必须在 summary 中说明已覆盖范围、未完成范围和仍需后续验证的候选线索。

如果当前只发现 1 个漏洞，默认不要结束。除非你已经明确完成主要攻击面覆盖，并能在 summary 中说明为什么没有更多可报告漏洞。

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

## 最终输出要求

当你完成审计后，必须调用 FinalizeFinding 工具提交最终结果；不要把最终 JSON 作为普通 assistant 文本输出。只有模型通道不支持工具调用时，才退化为输出严格 JSON。
FinalizeFinding 是终点工具。一旦调用成功，Finding 阶段会立即结束，因此只能在审计确实完成时调用。只有在以下条件满足时才允许调用 FinalizeFinding：
1. 已经完成主要攻击面审计，或已达到任务/轮次/上下文限制；
2. 每个放入 findings 的漏洞都已经补齐完整字段；
3. 不再存在明显需要继续验证的高价值候选；
4. 不要因为只找到第一个完整漏洞就提前结束，应继续搜索其他独立攻击面，直到证据充分或确认没有更多可报告漏洞。

FinalizeFinding 的参数必须是一个对象，包含：
- findings: 数组
- summary: 字符串
...
- 如果任一 finding 缺少 file_path、source、sink、exploit_chain、poc、impact、cve_justification 或 verification_notes，不要调用 FinalizeFinding，应优先继续调用 Read/Grep/Glob/Bash/PowerShell 等工具补齐证据；
- 如果最终仍无法闭合，只能在 summary 中简要说明已排除或证据不足，不能伪造成完整 finding。
...
class FindingAgent(AnalysisWorkflowAgent):
    ...
    async def _run_runtime_stack(self, input_data: Dict[str, Any]):
        ...
        bridge_result = await bridge.run(
            ...
            event_sink=self._build_runtime_activity_event_sink(),
        )
        ...
        if incomplete_runtime:
            error = self._format_incomplete_runtime_error(processed)
            return AgentResult(
                success=False,
                data=processed,
                error=error,
    ...
    def _build_runtime_activity_event_sink(self):
        pending_reasoning = ""
        last_emitted_assistant_content = ""

        async def flush_pending_reasoning(event_type: str, message: Dict[str, Any] | None = None) -> None:
            nonlocal pending_reasoning, last_emitted_assistant_content
            content = pending_reasoning.strip()
            if not content or content == last_emitted_assistant_content:
                return
            await self.emit_event(
                ...
                metadata={
                    ...
                    "audit_message_id": message.get("id") if isinstance(message, dict) else None,
                    "audit_message_sequence": message.get("sequence") if isinstance(message, dict) else None,
                },
            )
            last_emitted_assistant_content = content
            pending_reasoning = ""

        async def emit_runtime_activity(event: Dict[str, Any]) -> None:
            nonlocal pending_reasoning, last_emitted_assistant_content
            ...
            if event_type == "reasoning_delta":
                pending_reasoning = str(
                    ...
                    or (pending_reasoning + str((event or {}).get("content") or ""))
                ).strip()
                return

            if event_type == "done":
                content = self._extract_runtime_assistant_content(event)
                if not content:
                ...
                if retry_message:
                    await self.emit_event(
                        ...
                        metadata={
                            "source": "finding_runtime",
                            "runtime_event_type": event_type,
                            "attempt": (event or {}).get("attempt"),
                            "max_attempts": (event or {}).get("max_attempts"),
                            "error_type": (event or {}).get("error_type"),
                            "raw_error": (event or {}).get("error"),
                        },
                    )
                return
        ...
        return emit_runtime_activity

    @staticmethod
    def _extract_runtime_assistant_content(event: Dict[str, Any]) -> str:
        message = event.get("message") if isinstance(event, dict) else None
        message_payload = message.get("payload") if isinstance(message, dict) and isinstance(message.get("payload"), dict) else {}
        candidates = [
            event.get("accumulated") if isinstance(event, dict) else None,
            event.get("content") if isinstance(event, dict) else None,
            event.get("reasoning_content") if isinstance(event, dict) else None,
    ...
    @staticmethod
    def _format_incomplete_runtime_error(processed: Dict[str, Any]) -> str:
        runtime_error = processed.get("runtime_error")
...
- 持续审计，直到能证明一条 CVE 级利用链，或说明当前候选为什么达不到报告标准。"""
    ...
    def _postprocess_result(self, raw_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key in ("runtime_error", "is_final", "requires_retry", "runtime_completion_mode"):
            if key in raw_result:
                processed[key] = raw_result[key]
